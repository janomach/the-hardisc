...
## [1.1.0] - 2023-09-03 TODO:

### Added
 - Configurable protection of incoming and outcoming data for the instruction bus and data bus by EDAC.
 ...
 - A software-transparent Read-Modify-Write mechanism is integrated into the **LSU**, allowing a store of sub-word data to the EDAC-protected memory.
 - Maskable (non-maskable) interrupts for correctable (uncorrectable) errors at EDAC-protected interfaces.
 - The new **s_hold_o** signal is routed from the MA stage into the EX stage to signalize that the instruction should be terminated. This is necessary for faster interrupt handling and when the EDAC-pretected interface is enabled.
 - An assembly file with a Vector table is provided. Among other things, it also contains an example interrupt handler for correcting the correctable errors in memories with the help of the newly introduced **maddrerr** CSR register.
...
### Changed
 ...
 - The **IFB** entries are extended by 2 bits, which simplifies the propagation of error information and provides space for future functionalities.
 - The **IFB** provides a separate port which signalizes valid data at output
 - The **Aligner** is updated for compatibility with changes in the FE stage
 - A new pipeline register, **xxxx_IMISCON**, is introduced to propagate misconduct information through the pipeline. This means that the information is not propagated via **xxxx_ICTRL** register, which simplifies and makes the pipeline more clear.
 ...
 - The data bus transfer address propagates from the EX to the MA stage via **EXMA_VAL**, not **EXMA_PAYLOAD**. This allows reducing the width of **EXMA_PAYLOAD** register to 12 bits only.
 - The branch offset for update of the predictor is stored into a new 20-bit register **EXMA_OFFST**, which does not need to be replicated in the protected version of the core.
 ...
 - The data bus exception logic is moved from MA stage into the **LSU** module.
 - An aligment of the loaded data in the **lsu_decoder** is refined.
 - The evaluation of exceptions and selection of the exception code is moved from MA stage module into the **csr_executor** module.
