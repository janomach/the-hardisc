...
hardiscSim:
	vsim -${HOW} ${HARDISC_SIM}/work.tb_mh_wrapper +TIMEOUT=${SIM_TIMEOUT} +BIN=${BINARY} +LOGGING=${LOGGING} +SEE_PROB=${SEE_PROB} +SEE_GROUP=${SEE_GROUP} +LAT=${LAT} -sv_seed ${SV_SEED} +ECALLHALT=${ECALLHALT} +LFILE=$(HARDISC_DIR)/hardisc_${TIME_STAMP}.log +PFILE=$(HARDISC_DIR)/prof_${TIME_STAMP}.log -do "do ${HARDISC_DIR}/scripts/basic_waves_mh.do; run 100ms"

compileTest:
	riscv64-unknown-elf-gcc -o ${TEST_DIR}/test.o ${TEST_DIR}/test.c ${SYSCALLS} -T ${LD_SCRIPT} ${VECTORS} ${STARTUP} ${CFLAGS} -I ${RISCV}/include -L ${RISCV}/lib -lc -lm -lgcc
	riscv64-unknown-elf-objcopy -O binary ${TEST_DIR}/test.o ${TEST_DIR}/test.bin
	riscv64-unknown-elf-objdump --disassemble ${TEST_DIR}/test.o > ${TEST_DIR}/test.txt
